...
int main(){
	...
	worms.push_back({0, 0});
	worms.push_back(mp(1, v[0]));
	for(int i = 1;i < n; ++i){
		worms.push_back(mp((worms[i].second + 1), (v[i] + (worms[i].second))));
	}
	cin >> m;
	while(m--){
		cin >> q;
		...
		while(l <= r){
			int mid = (l + r + 1)  / 2;
			if(worms[mid].first <= q && worms[mid].second >= q){
				cout << mid << endl;
				break;
			}
			else if(worms[mid].first < q && worms[mid].second < q){
				l = mid + 1;
			}
			else{
				r = mid - 1;
